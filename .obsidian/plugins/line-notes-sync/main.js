...
var CryptoUtils = class {
  ...
  static getPlatformFingerprint() {
    const parts = [];
    if (import_obsidian.Platform.isMacOS) parts.push("macos");
    if (import_obsidian.Platform.isWin) parts.push("win");
    if (import_obsidian.Platform.isLinux) parts.push("linux");
    if (import_obsidian.Platform.isIosApp) parts.push("ios");
    if (import_obsidian.Platform.isAndroidApp) parts.push("android");
    if (import_obsidian.Platform.isMobileApp) parts.push("mobile");
    if (import_obsidian.Platform.isDesktopApp) parts.push("desktop");
...
var MessageEncryptor = class {
  ...
  isEncryptedMessage(message) {
    ...
    const m = message;
    return typeof m.encryptedContent === "string" && typeof m.encryptedAESKey === "string" && typeof m.iv === "string" && typeof m.version === "string";
  }
  /**
    ...
    if (m.encrypted === true && typeof m.encryptedContent === "string") {
      try {
        const encryptedMessage = {
          ...
          encryptedAESKey: String((_b = (_a = m.encryptedAESKey) != null ? _a : m.encryptedAesKey) != null ? _b : ""),
          iv: String((_c = m.iv) != null ? _c : ""),
          senderKeyId: String((_d = m.senderKeyId) != null ? _d : ""),
          recipientUserId: String((_e = m.recipientUserId) != null ? _e : ""),
...
var E2EEErrorHandler = class {
  ...
  async handleKeyNotInitialized() {
    try {
      ...
      new import_obsidian2.Notice("\u63A5\u7D9A\u30A8\u30E9\u30FC\u304C\u767A\u751F\u3057\u307E\u3057\u305F\u3002\u8A2D\u5B9A\u3092\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002");
      throw new E2EEError(
   ...
   */
  async handleDecryptionFailed(_error) {
    return "[\u30E1\u30C3\u30BB\u30FC\u30B8\u3092\u8AAD\u307F\u8FBC\u3081\u307E\u305B\u3093\u3067\u3057\u305F]";
  }
  /**
  ...
  async handleNetworkError(error) {
    new import_obsidian2.Notice("\u30CD\u30C3\u30C8\u30EF\u30FC\u30AF\u30A8\u30E9\u30FC\u304C\u767A\u751F\u3057\u307E\u3057\u305F\u3002\u30A4\u30F3\u30BF\u30FC\u30CD\u30C3\u30C8\u63A5\u7D9A\u3092\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002");
    throw error;
  }
  /**
  ...
  handleGenericError(error) {
    new import_obsidian2.Notice("\u51E6\u7406\u4E2D\u306B\u30A8\u30E9\u30FC\u304C\u767A\u751F\u3057\u307E\u3057\u305F");
    throw error;
  }
  /**
...
var LinePlugin = class extends import_obsidian3.Plugin {
  constructor() {
    super(...arguments);
  ...
  async generateUniqueFileName(message, folderPath) {
    ...
    let counter = 1;
    while (true) {
      const fullPath = (0, import_obsidian3.normalizePath)(`${folderPath}/${uniqueFileName}`);
  ...
  async fetchSubscriptionStatus() {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        this.settings.voiceCount = (_a = data.voiceCount) != null ? _a : 0;
        this.settings.voiceFreeLimit = (_b = data.voiceFreeLimit) != null ? _b : 10;
  ...
  async syncMessages(isAutoSync = false) {
    if (!this.settings.vaultId) {
      new import_obsidian3.Notice("Vault ID not configured. Please set it in plugin settings.");
      return;
    }
    if (this.settings.lineUserId) {
    ...
    try {
      if (!isAutoSync) {
        new import_obsidian3.Notice("Syncing LINE messages...");
      }
      ...
      if (this.settings.groupMessagesByDate) {
        ...
        for (const [dateString, dateMessages] of messagesByDate) {
          ...
          try {
            const normalizedFolderPath = (0, import_obsidian3.normalizePath)(this.settings.noteFolderPath);
            ...
            const normalizedTargetFolderPath = (0, import_obsidian3.normalizePath)(folderPath);
            ...
            const normalizedFilePath = (0, import_obsidian3.normalizePath)(filePath);
            let existingContent = "";
            ...
            for (const message of dateMessages) {
              ...
              try {
                ...
                try {
                  const handled = await this.errorHandler.handleError(error, `message_${message.messageId}`);
                  messageText = handled != null ? handled : message.text || "[\u30E1\u30C3\u30BB\u30FC\u30B8\u3092\u8AAD\u307F\u8FBC\u3081\u307E\u305B\u3093\u3067\u3057\u305F]";
                } catch (e) {
            ...
            if (fileToWrite instanceof import_obsidian3.TFile) {
      ...
      if (newMessageCount > 0 || newImageCount > 0 || !isAutoSync) {
        ...
        new import_obsidian3.Notice(`LINE sync completed. ${summary}.`);
      }
    } catch (err) {
      new import_obsidian3.Notice(`Failed to sync LINE messages: ${err instanceof Error ? err.message : "Unknown error"}`);
    }
  }
  async syncImages() {
    try {
      ...
      for (const image of images) {
        ...
        try {
          ...
          const contentResponse = await (0, import_obsidian4.requestUrl)({
            url: contentUrl,
            method: "GET"
          });
          ...
          const normalizedImageFolderPath = (0, import_obsidian3.normalizePath)(this.settings.imageFolderPath);
          ...
          const imageFilePath = (0, import_obsidian3.normalizePath)(`${this.settings.imageFolderPath}/${imageFileName}`);
  ...
  async createImageNote(image, imageFileName) {
    ...
    const normalizedNoteFolderPath = (0, import_obsidian3.normalizePath)(noteFolderPath);
    const noteFolder = this.app.vault.getAbstractFileByPath(normalizedNoteFolderPath);
    if (!noteFolder) {
      const normalizedBaseFolderPath = (0, import_obsidian3.normalizePath)(this.settings.noteFolderPath);
    ...
    if (this.settings.groupMessagesByDate) {
      ...
      const notePath = (0, import_obsidian3.normalizePath)(`${noteFolderPath}/${noteFileName}`);
      const content = [
        `---`,
        `source: LINE`,
  ...
  async registerCurrentPublicKey() {
    try {
      await this.keyManager.initialize();
    } catch (error) {
      const keys = await this.keyManager.loadKeys();
  ...
  async registerMapping() {
    ...
    if (!lineUserId || !vaultId) {
      new import_obsidian3.Notice("LINE UserID\u3068Vault ID\u306E\u4E21\u65B9\u3092\u8A2D\u5B9A\u3057\u3066\u304F\u3060\u3055\u3044\u3002");
      return;
    }
    ...
    try {
      ...
      try {
        ...
        new import_obsidian3.Notice(`\u30DE\u30C3\u30D4\u30F3\u30B0\u306F\u767B\u9332\u3055\u308C\u307E\u3057\u305F\u304C\u3001E2EE\u516C\u958B\u9375\u306E\u767B\u9332\u306B\u5931\u6557\u3057\u307E\u3057\u305F: ${keyError instanceof Error ? keyError.message : "Unknown error"}`);
        return;
      }
      new import_obsidian3.Notice("LINE UserID\u3068Vault ID\u306E\u30DE\u30C3\u30D4\u30F3\u30B0\u3068E2EE\u516C\u958B\u9375\u3092\u767B\u9332\u3057\u307E\u3057\u305F\u3002");
    } catch (error) {
      new import_obsidian3.Notice(`\u30DE\u30C3\u30D4\u30F3\u30B0\u306E\u767B\u9332\u306B\u5931\u6557\u3057\u307E\u3057\u305F: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }
  async resetMapping() {
    if (!this.settings.lineUserId || !this.settings.vaultId) {
    ...
    try {
      ...
      this.settings.lineUserId = "";
      await this.saveSettings();
      new import_obsidian3.Notice("LINE UserID\u3068Vault ID\u306E\u30DE\u30C3\u30D4\u30F3\u30B0\u3092\u30EA\u30BB\u30C3\u30C8\u3057\u307E\u3057\u305F\u3002");
    } catch (error) {
      new import_obsidian3.Notice(`\u30DE\u30C3\u30D4\u30F3\u30B0\u306E\u30EA\u30BB\u30C3\u30C8\u306B\u5931\u6557\u3057\u307E\u3057\u305F: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }
};
var ConfirmResetModal = class extends import_obsidian3.Modal {
...
var LineSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian3.Setting(containerEl).setName("Note folder path").setDesc("LINE\u30E1\u30C3\u30BB\u30FC\u30B8\u304C\u4FDD\u5B58\u3055\u308C\u308B\u30D5\u30A9\u30EB\u30C0\u30D1\u30B9").addText((text) => text.setPlaceholder("LINE").setValue(this.plugin.settings.noteFolderPath).onChange(async (value) => {
      this.plugin.settings.noteFolderPath = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian3.Setting(containerEl).setName("Vault ID").setDesc("\u3053\u306EObsidian Vault\u7528\u306E\u4E00\u610F\u306E\u8B58\u5225\u5B50\uFF08\u4EFB\u610F\u306E\u30E6\u30CB\u30FC\u30AF\u306AID\u3092\u4F5C\u6210\u3057\u3066\u304F\u3060\u3055\u3044\uFF09").addText((text) => text.setPlaceholder("Enter vault ID").setValue(this.plugin.settings.vaultId).onChange(async (value) => {
      this.plugin.settings.vaultId = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian3.Setting(containerEl).setName("LINE user ID").setDesc("LINE\u30DC\u30C3\u30C8\u3068\u306E\u4F1A\u8A71\u3067\u53D6\u5F97\u3057\u305F\u30E6\u30FC\u30B6\u30FCID\u3092\u5165\u529B\u3057\u3066\u304F\u3060\u3055\u3044").addText((text) => text.setPlaceholder("Enter your LINE User ID").setValue(this.plugin.settings.lineUserId).onChange(async (value) => {
      this.plugin.settings.lineUserId = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian3.Setting(containerEl).setName("Auto sync").setDesc("LINE\u30E1\u30C3\u30BB\u30FC\u30B8\u3092\u81EA\u52D5\u7684\u306B\u540C\u671F\u3059\u308B\u304B\u3069\u3046\u304B").addToggle((toggle) => toggle.setValue(this.plugin.settings.autoSync).onChange(async (value) => {
      this.plugin.settings.autoSync = value;
      await this.plugin.saveSettings();
      syncIntervalSetting.settingEl.toggle(value);
    }));
    const syncIntervalSetting = new import_obsidian3.Setting(containerEl).setName("Sync interval").setDesc("LINE\u30E1\u30C3\u30BB\u30FC\u30B8\u3092\u540C\u671F\u3059\u308B\u9593\u9694\uFF08\u6642\u9593\u5358\u4F4D\uFF09").addDropdown((dropdown) => {
    ...
    new import_obsidian3.Setting(containerEl).setName("Sync on startup").setDesc("Obsidian\u8D77\u52D5\u6642\u306BLINE\u30E1\u30C3\u30BB\u30FC\u30B8\u3092\u540C\u671F\u3059\u308B\u304B\u3069\u3046\u304B").addToggle((toggle) => toggle.setValue(this.plugin.settings.syncOnStartup).onChange(async (value) => {
      this.plugin.settings.syncOnStartup = value;
      await this.plugin.saveSettings();
    }));
    let organizeBydateToggle;
    new import_obsidian3.Setting(containerEl).setName("Organize by date").setDesc("\u65E5\u4ED8\u3054\u3068\u306B\u30D5\u30A9\u30EB\u30C0\u3092\u4F5C\u6210\u3057\u3066\u30E1\u30C3\u30BB\u30FC\u30B8\u3092\u6574\u7406\u3059\u308B\u304B\u3069\u3046\u304B\uFF08\u6CE8\u610F\uFF1A\u300CGroup messages by date\u300D\u3092\u30AA\u30F3\u306B\u3059\u308B\u3068\u81EA\u52D5\u7684\u306B\u30AA\u30D5\u306B\u306A\u308A\u307E\u3059\u304C\u3001\u624B\u52D5\u3067\u518D\u5EA6\u30AA\u30F3\u306B\u3059\u308B\u3053\u3068\u304C\u3067\u304D\u307E\u3059\uFF09").addToggle((toggle) => {
    ...
    new import_obsidian3.Setting(containerEl).setHeading().setName("\u30D5\u30A1\u30A4\u30EB\u6574\u7406\u8A2D\u5B9A");
    new import_obsidian3.Setting(containerEl).setName("Group messages by date").setDesc("\u540C\u3058\u65E5\u4ED8\u306E\u30E1\u30C3\u30BB\u30FC\u30B8\u30921\u3064\u306E\u30D5\u30A1\u30A4\u30EB\u306B\u307E\u3068\u3081\u308B\u304B\u3069\u3046\u304B\uFF08\u30C1\u30A7\u30C3\u30AF\u3092\u5916\u3059\u3068\u30E1\u30C3\u30BB\u30FC\u30B8\u3054\u3068\u306B\u500B\u5225\u306E\u30D5\u30A1\u30A4\u30EB\u3092\u4F5C\u6210\uFF09").addToggle((toggle) => toggle.setValue(this.plugin.settings.groupMessagesByDate).onChange(async (value) => {
    ...
    const messageTemplateSetting = new import_obsidian3.Setting(containerEl).setName("Grouped message template").setDesc("\u65E5\u4ED8\u3067\u30B0\u30EB\u30FC\u30D7\u5316\u3055\u308C\u305F\u30E1\u30C3\u30BB\u30FC\u30B8\u306E\u8868\u793A\u30C6\u30F3\u30D7\u30EC\u30FC\u30C8\n\u5229\u7528\u53EF\u80FD\u306A\u5909\u6570: {time} - \u6642\u523B, {text} - \u30E1\u30C3\u30BB\u30FC\u30B8\u5185\u5BB9, {messageId} - \u30E1\u30C3\u30BB\u30FC\u30B8ID, {userId} - \u30E6\u30FC\u30B6\u30FCID").addTextArea((text) => {
    ...
    const frontmatterTemplateSetting = new import_obsidian3.Setting(containerEl).setName("Grouped message frontmatter template").setDesc("\u65E5\u4ED8\u3067\u30B0\u30EB\u30FC\u30D7\u5316\u3055\u308C\u305F\u30D5\u30A1\u30A4\u30EB\u306E\u30D5\u30ED\u30F3\u30C8\u30DE\u30BF\u30FC\n\u5229\u7528\u53EF\u80FD\u306A\u5909\u6570: {date} - \u65E5\u4ED8, {datecompact} - \u65E5\u4ED8\uFF08\u30CF\u30A4\u30D5\u30F3\u306A\u3057\uFF09").addTextArea((text) => {
    ...
    new import_obsidian3.Setting(containerEl).setHeading().setName("\u753B\u50CF\u8A2D\u5B9A");
    new import_obsidian3.Setting(containerEl).setName("Sync images").setDesc("LINE\u3067\u9001\u4FE1\u3057\u305F\u753B\u50CF\u3092\u540C\u671F\u3059\u308B\u304B\u3069\u3046\u304B").addToggle((toggle) => toggle.setValue(this.plugin.settings.syncImages).onChange(async (value) => {
    ...
    const imageFolderSetting = new import_obsidian3.Setting(containerEl).setName("Image folder path").setDesc("\u753B\u50CF\u30D5\u30A1\u30A4\u30EB\u306E\u4FDD\u5B58\u5148\u30D5\u30A9\u30EB\u30C0").addText((text) => text.setPlaceholder("LINE/images").setValue(this.plugin.settings.imageFolderPath).onChange(async (value) => {
    ...
    new import_obsidian3.Setting(containerEl).setHeading().setName("\u30D7\u30EC\u30DF\u30A2\u30E0\u30D7\u30E9\u30F3");
    const statusText = this.getSubscriptionStatusText();
    const statusSetting = new import_obsidian3.Setting(containerEl).setName("\u73FE\u5728\u306E\u30D7\u30E9\u30F3").setDesc(statusText);
    ...
    new import_obsidian3.Setting(containerEl).setName("\u30B5\u30D6\u30B9\u30AF\u30EA\u30D7\u30B7\u30E7\u30F3\u72B6\u614B\u3092\u66F4\u65B0").setDesc("\u30B5\u30FC\u30D0\u30FC\u304B\u3089\u6700\u65B0\u306E\u30B5\u30D6\u30B9\u30AF\u30EA\u30D7\u30B7\u30E7\u30F3\u72B6\u614B\u3092\u53D6\u5F97\u3057\u307E\u3059").addButton((button) => button.setButtonText("\u66F4\u65B0").onClick(async () => {
      button.setDisabled(true);
      button.setButtonText("\u66F4\u65B0\u4E2D...");
      try {
        await this.plugin.fetchSubscriptionStatus();
        new import_obsidian3.Notice("\u30B5\u30D6\u30B9\u30AF\u30EA\u30D7\u30B7\u30E7\u30F3\u72B6\u614B\u3092\u66F4\u65B0\u3057\u307E\u3057\u305F");
        this.display();
      } catch (e) {
        new import_obsidian3.Notice("\u30B5\u30D6\u30B9\u30AF\u30EA\u30D7\u30B7\u30E7\u30F3\u72B6\u614B\u306E\u53D6\u5F97\u306B\u5931\u6557\u3057\u307E\u3057\u305F");
      } finally {
        button.setDisabled(false);
        button.setButtonText("\u66F4\u65B0");
      }
    }));
    new import_obsidian3.Setting(containerEl).setHeading().setName("\u63A5\u7D9A\u8A2D\u5B9A");
    new import_obsidian3.Setting(containerEl).setName("Register mapping").setDesc("LINE UserID\u3068Vault ID\u306E\u30DE\u30C3\u30D4\u30F3\u30B0\u3068E2EE\u516C\u958B\u9375\u3092\u767B\u9332\u3057\u307E\u3059\u3002\u30E1\u30C3\u30BB\u30FC\u30B8\u3092\u8AAD\u307F\u8FBC\u3081\u306A\u3044\u5834\u5408\u3082\u3053\u306E\u64CD\u4F5C\u3067\u63A5\u7D9A\u3092\u4FEE\u5FA9\u3067\u304D\u307E\u3059").addButton((button) => button.setButtonText("Register").onClick(async () => {
      await this.plugin.registerMapping();
    }));
    new import_obsidian3.Setting(containerEl).setHeading().setName("\u8A73\u7D30\u8A2D\u5B9A");
    new import_obsidian3.Setting(containerEl).setName("Reset mapping").setDesc("LINE UserID\u3068Vault ID\u306E\u30DE\u30C3\u30D4\u30F3\u30B0\u3092\u30EA\u30BB\u30C3\u30C8\u3057\u307E\u3059\uFF08KV\u4E0A\u306E\u7D10\u3065\u3051\u3092\u524A\u9664\u3057\u3001\u518D\u767B\u9332\u53EF\u80FD\u306B\u3057\u307E\u3059\uFF09").addButton((button) => button.setButtonText("Reset").setWarning().onClick(() => {
